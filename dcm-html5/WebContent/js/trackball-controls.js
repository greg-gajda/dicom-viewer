...
THREE.TrackballControls = function(object, domElement) {
	...
	var getMouseOnScreen = (function() {
		...
		return function getMouseOnScreen(pageX, pageY) {
			vector.set((pageX - _this.screen.left) / _this.screen.width,
	...
	this.rotateCamera = (function() {
		...
		return function rotateCamera() {
			moveDirection.set(_moveCurr.x - _movePrev.x, _moveCurr.y
					- _movePrev.y, 0);
			...
			if (angle) {
				...
				objectSidewaysDirection.crossVectors(objectUpDirection,
						eyeDirection).normalize();
				objectUpDirection.setLength(_moveCurr.y - _movePrev.y);
				objectSidewaysDirection.setLength(_moveCurr.x - _movePrev.x);
				moveDirection.copy(objectUpDirection
						.add(objectSidewaysDirection));
	...
	this.panCamera = (function() {
		...
		return function panCamera() {
			...
			if (mouseChange.lengthSq()) {
				...
				pan
						.add(objectUp.copy(_this.object.up).setLength(
								mouseChange.y));
		...
		if (!_this.noZoom || !_this.noPan) {
			if (_eye.lengthSq() > _this.maxDistance * _this.maxDistance) {
				_this.object.position.addVectors(_this.target, _eye
						.setLength(_this.maxDistance));
				_zoomStart.copy(_zoomEnd);
			}
			if (_eye.lengthSq() < _this.minDistance * _this.minDistance) {
				_this.object.position.addVectors(_this.target, _eye
						.setLength(_this.minDistance));
				_zoomStart.copy(_zoomEnd);
			}
		}
	};
	...
	function touchstart(event) {
		...
		case 1:
			...
			_moveCurr.copy(getMouseOnCircle(event.touches[0].pageX,
	...
	this.dispose = function() {
		...
		this.domElement.removeEventListener('MozMousePixelScroll', mousewheel,
				false); // firefox
...
THREE.TrackballControls.prototype = Object
		.create(THREE.EventDispatcher.prototype);
