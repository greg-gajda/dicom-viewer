...
public class RectifyUShortToShortDataDescriptor extends OperationDescriptorImpl implements RenderedImageFactory {
     ...
     */
    private static final String[][] resources = { { "GlobalName", "RectifyUShortToShortData" },  

        { "LocalName", "RectifyUShortToShortData" },  

        { "Vendor", "" },  

        { "Description", 
            "Rectify image with unsigned short data to signed short data (Workaround for imageio codecs issue" }, 
    ...
    public RenderedImage create(ParameterBlock args, RenderingHints renderHints) {
        ...
        if(renderHints != null){
        	renderHints = RenderingHints.class.cast(renderHints.get(JAI.KEY_IMAGE_LAYOUT));
        }
    ...
    public static RenderedOp create(RenderedImage source0, RenderingHints hints) {
        ParameterBlockJAI pb = new ParameterBlockJAI("RectifyUShortToShortData", RenderedRegistryMode.MODE_NAME); 

        pb.setSource("source0", source0); 

        return JAI.create("RectifyUShortToShortData", pb, hints); 
    }

    public static RenderableOp createRenderable(RenderableImage source0, RenderingHints hints) {
        ParameterBlockJAI pb = new ParameterBlockJAI("RectifyUShortToShortData", RenderableRegistryMode.MODE_NAME); 

        pb.setSource("source0", source0); 

        return JAI.createRenderable("RectifyUShortToShortData", pb, hints); 
    }
}
